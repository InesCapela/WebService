...
public class ExplicadorController {
    ...
    public ResponseEntity<Explicador> getExplicadorByNome(@PathVariable("nome_explicador") String nome) throws NoExplicadorNomeExcpetion{
        this.logger.info("Received a get request");

        Optional<Explicador> explicadorOptional = this.explicadorService.findByName(nome);
        if (explicadorOptional.isPresent()) {
            return ResponseEntity.ok(explicadorOptional.get());
        }
    ...
    private static class NoExplicadorNomeExcpetion extends RuntimeException {

        public NoExplicadorNomeExcpetion(String name) {
            super("No such explicador with name: " + name);
        }
    }
